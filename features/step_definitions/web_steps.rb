...
When (/^client requests GET (.*)$/) do |path|
  @response = HTTParty.get('http://localhost:4567' + path)
end

When (/^client requests POST (.*)$/) do |path|
  @response = HTTParty.post('http://localhost:4567' + path)
end

Given (/^valid recipe id$/) do 
  @recipe_id = 1
end 

Then (/^return a recipe object that includes (.+)$/) do |expected_json|
  # expected = JSON.pretty_generate(JSON.parse(expected_json))
